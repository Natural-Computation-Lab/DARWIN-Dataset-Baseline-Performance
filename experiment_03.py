...
the experiment tests only 1 dataset division in training and test.
...
for random_state in random_states:
    ...
    if first_seed_run:
        initial_task = START_TASK
        first_seed_run = False
    else:
        initial_task = 0
    print("----> Seed:", random_state)
    for task in range(initial_task,25):
        print("  Task:", task+1)
        ...
        if ranFor:
            ...
            confusion_m = confusion_matrix(y_test, predictions)
            conf_m = [("", "A", "P"), ("A", confusion_m[0][0], confusion_m[0][1]),
                      ("P", confusion_m[1][0], confusion_m[1][1])]

            add_modelresults_to_report(name, scoring, grid_search, grid_search.best_score_, y_test, predictions,
                                       predictions_TS)

            file_txt.write("Best model:\n" + str(best_model).rstrip("\n") + "\n\n")
            file_txt.write("confusion matrix:\n")
            for line in conf_m:
                for el in line:
